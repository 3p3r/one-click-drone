...
const agw = require('@aws-cdk/aws-apigateway');
...
class Stack extends cdk.Stack {
  ...
  constructor(scope, id, props) {
    ...
    // Add a proxy API Gateway to wrap Drone's port 80 for security and metering purposes
    const elbBaseUrl = cdk.Fn.join('', ['http://', server.loadBalancer.loadBalancerDnsName]);
    const elbProxyUrl = cdk.Fn.join('', [elbBaseUrl, '/{proxy}']);
    const api = new agw.RestApi(this, 'https-wrapper', {
    ...
    api.root.addMethod('GET', new agw.HttpIntegration(elbBaseUrl));
    api.root.addProxy({
      ...
      defaultIntegration: new agw.HttpIntegration(elbProxyUrl, {
        httpMethod: 'ANY',
        options: {
          requestParameters: {
    ...
    new cdk.CfnOutput(this, 'OneClickDroneHost', { value: api.urlForPath() });
  }
}
