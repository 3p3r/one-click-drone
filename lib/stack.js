...
class Stack extends cdk.Stack {
  ...
  constructor(scope, id, props) {
    ...
    const cacheVolume = new efs.CfnFileSystem(this, 'cache-volume', {
    ...
    // Create a security group that opens up the database to our subnet
    const efsSecurityGroup = new ec2.SecurityGroup(this, 'efs-security-group', { allowAllOutbound: true, vpc });
    ...
    vpc.privateSubnets.forEach((subnet, index) => {
      new efs.CfnMountTarget(this, `cache-volume-mt-${index}`, {
        fileSystemId: cacheVolume.ref,
        subnetId: subnet.subnetId,
        securityGroups: [efsSecurityGroup.securityGroupId],
      });
    });
    // Add our cache EFS volume to task definition of runners
    ...
    runner.taskDefinition.addVolume({
      ...
      dockerVolumeConfiguration: {
        ...
        driverOpts: {
          ...
          o: cdk.Fn.sub(
            [
              'addr=${efsId}.efs.${region}.amazonaws.com',
              'nfsvers=4.1',
              'rsize=1048576',
              'wsize=1048576',
              'hard',
    ...
    // mount EFS in all containers under /cache
    runner.taskDefinition.defaultContainer.addMountPoints({
